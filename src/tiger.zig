...
pub const Tiger = struct {
    ...
    pub fn compress(self: *Self, block: *const [64]u8) void {
        // Load block into 8 64-bit words (little-endian)
        var x: [8]u64 = undefined;
        var i: usize = 0;
        while (i < 8) : (i += 1) {
            x[i] = std.mem.readInt(u64, block[i * 8 ..][0..8], .little);
        }
    ...
    fn round(a: *u64, b: *u64, c: *u64, x: u64, mul: u64) void {
        ...
        b.* +%= t4[@as(u8, @truncate(c.* >> 8))] ^ t3[@as(u8, @truncate(c.* >> 24))] ^ t2[@as(u8, @truncate(c.* >> 40))] ^ t1[@as(u8, @truncate(c.* >> 56))];
        b.* *%= mul;
    }
